// ============================================================
// Felix Optional Module — Atlas DeFi Monitoring
// ============================================================
// This module is DISABLED by default.
// It will not load or execute any code unless the environment
// variable ENABLE_FELIX=true is explicitly set.
//
// When disabled, isFelixEnabled() returns false and
// getFelixModule() returns null. Nothing is imported.
//
// To enable: set ENABLE_FELIX=true in your environment or
// Supabase edge function secrets before deploying.
// ============================================================
...
// ─── Guard: do not activate unless ENABLE_FELIX=true ─────────────────────────

const _felixEnabled = Deno.env.get("ENABLE_FELIX") === "true";

/**
 * Returns true only when ENABLE_FELIX=true is set in the environment.
 * Use this check before calling getFelixModule() to avoid null-reference errors.
 */
export function isFelixEnabled(): boolean {
  return _felixEnabled;
}

/**
 * Returns the fully-initialized Felix plugin object, or null if the module
 * is disabled. The plugin object is never constructed unless ENABLE_FELIX=true.
 */
export function getFelixModule(): Plugin | null {
  if (!_felixEnabled) {
    return null;
  }
  return _buildFelixPlugin();
}

// ─── Everything below only executes when getFelixModule() is called ───────────
// ─── and ENABLE_FELIX=true. No side-effects at import time.          ───────────

if (_felixEnabled) {
  console.log("[felix-optional] ENABLE_FELIX=true detected. Felix module is active.");
} else {
  console.log("[felix-optional] Felix module is disabled. Set ENABLE_FELIX=true to activate.");
}
...
const FELIX_CONTRACT  = Deno.env.get("FELIX_CONTRACT")      ?? "";
const FELIX_CHAIN     = Deno.env.get("FELIX_CHAIN")         ?? "ethereum";
const FELIX_COINGECKO = Deno.env.get("FELIX_COINGECKO_ID")  ?? "felix-token";
...
// ─── Plugin builder (only called when ENABLE_FELIX=true) ─────────────────────

function _buildFelixPlugin(): Plugin {
  return {
    name: "atlas-felix",
...
// ─── Default export: null when disabled, plugin when enabled ──────────────────

const atlasFelix: Plugin | null = _felixEnabled ? _buildFelixPlugin() : null;
