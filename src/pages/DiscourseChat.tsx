...
const PAGE_SIZE = 30;
const DISCOURSE_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/discourse`;
...
const DiscourseChat = () => {
  ...
  const loadSidebar = useCallback(async () => {
    ...
    } catch { /* tables may not exist — sidebar stays empty */ }
  }, [user]);
  ...
  useEffect(() => {
    if (isInitialLoad.current && historyLoaded) {
      ...
      if (el) {
        ...
        if (nearBottom || streaming) bottomRef.current?.scrollIntoView({ behavior: "smooth" });
      }
    }
  ...
  const sendMessage = useCallback(async (overrideText?: string) => {
    ...
    try {
      ...
      console.error("[DiscourseChat] error:", msg);
    ...
    <div className="flex h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto py-2 space-y-0.5 min-h-0">
          ...
          <div className="px-3 py-1.5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowNewProject((v) => !v)} className="text-muted-foreground/40 hover:text-accent transition-colors" title="New project">
              ...
              <div key={proj.id}>
                ...
                {isOpen && (
                  <div className="ml-5">
                    ...
                    {convs.length === 0 && <p className="px-3 py-1 text-[10px] text-muted-foreground/30">No chats yet</p>}
                  </div>
                )}
              </div>
            );
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="h-10 flex items-center px-4 border-b border-border/30 gap-3 shrink-0">
          <button onClick={() => setSidebarOpen((v) => !v)} className="text-muted-foreground/50 hover:text-primary transition-colors" title="Toggle conversation list">
            <MoreHorizontal className="h-4 w-4" />
          </button>
          <span className="text-xs font-display tracking-widest text-primary truncate">
            {currentConvId
              ? (conversations.find((c) => c.id === currentConvId)?.title ?? "Discourse")
              : "Discourse"}
          </span>
        </div>
          ...
          {!currentConvId && historyLoaded && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-6 text-center py-20">
              <div className="space-y-2">
                <p className="text-2xl font-display text-primary tracking-widest">DISCOURSE</p>
                <p className="text-sm text-muted-foreground/60 max-w-sm">
                  Your intelligence partner. What's on your mind?
                </p>
              </div>
          ...
          {streaming && streamContent && (
            <div className="flex gap-3 max-w-3xl">
              <div className="w-7 h-7 rounded-full bg-accent/20 border border-accent/30 flex items-center justify-center shrink-0 mt-0.5">
                <span className="text-[9px] font-display text-accent">D</span>
              </div>
